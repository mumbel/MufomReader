...
public abstract class MufomRecord {
	...
	public void reset(BinaryReader reader) throws IOException {
		reader.setPointerIndex(record_offset);
	}
	...
	public static void hexdump(BinaryReader reader, long position, long length) throws IOException {
		...
		for (long i = 0; i < length; i++) {
			x = x + String.format(" %02x", 0xff & reader.readUnsignedByte(start + i));
			if (((i + 1) % 16) == 0) x = x + '\n';
		}
		x = x + '\n';
	...
	protected void isAsciiPrintable(String str) throws IOException {
		if (null == str) {
			throw new IOException();
		}
		int sz = str.length();
	...
	protected String read_string(BinaryReader reader, boolean must) throws IOException {
		...
		if (0x7f >= len) {
			...
			if (must) {
				Msg.info(this, "Failed to read string " + len);
				if (do_debug) hexdump(reader, record_offset, 0x10);
				throw new IOException();
			}
			return null;
